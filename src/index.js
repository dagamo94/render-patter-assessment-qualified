...
function calculateTotal() {
  let total = 0;
  books.forEach((book) => (total += book.quantity * book.price));
...
function render() {
  // *** Render Cart Items ****
  const itemsSection = document.querySelector("#cartItems");
  ...
  // *** Render Total ***
  const totalSection = document.querySelector("#cartTotal > .total-price");
  totalSection.innerText = `$${calculateTotal()}`;
}
